...
This module is **testing-only**. The two working applications you must test are delivered as **testing templates** — self-contained, runnable repositories that already include the complete application source, the test tooling configuration, and empty test skeletons:

1. **Frontend testing template** — a SwapLoop **multi-page rider UI** (Module D behaviour) talking to a Module C–compatible API, with **Cypress** test skeletons.
2. **Backend testing template** — a **partner billing service** (Module B billing ideas) implemented in PHP, with **PHPUnit** test skeletons.
...
### What you receive — the testing templates

Each template contains the **application under test** (do not modify it) plus the test skeletons you fill in.

| Testing template                                                                                                      | Application under test                                | Handout                                                                            | Your work                            |
| --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------- |
| [Frontend testing template](https://github.com/sitc2026-dev/s17-sitc2026-module-g-frontend-testing-template) (Cypress) | Rider UI in `public/` + mock Module C API in `server.js` | [`assets/frontend-testing-template.zip`](./assets/frontend-testing-template.zip) | Implement specs under `cypress/e2e/` |
| [Backend testing template](https://github.com/sitc2026-dev/s17-sitc2026-module-g-backend-testing-template) (PHPUnit)   | Partner billing domain code in `src/` + `run.php`        | [`assets/backend-testing-template.zip`](./assets/backend-testing-template.zip)   | Implement tests under `tests/`       |

Both templates ship a `README.md` with their own setup and run instructions; each is runnable **with or without Docker**.

### Running the testing templates

**Frontend testing template** (Node 22+):

```bash
npm install            # Cypress 15.10.0
npm start              # serves the rider UI + mock API on http://localhost:3000
npm test               # headless Cypress run (second terminal)
npm run cypress:open   # interactive runner
```

**Backend testing template** (PHP 8.3+ and Composer):

```bash
composer install       # installs PHPUnit (dev tooling only)
php run.php            # demo billing export
composer test          # or: php vendor/bin/phpunit
```

Docker Compose files are included in both templates for competitors who prefer containers.
...
The PHP package in the **backend testing template** calculates partner subscription billing (Module B ideas) **without a database** and **without third-party runtime libraries** — Composer is present for dev tooling (PHPUnit) only, and the application never loads `vendor/autoload.php`. `BillingService` uses PHP `date()` and `file_put_contents()`, then notifies via a concrete `Notifier` class (no interface). Domain code uses **inheritance** and a **factory**.
...
Assessment runs the testing templates' applications unchanged and evaluates the competitor’s tests against the expectations above (automated where possible, expert review of coverage and assertion quality). Empty or incomplete cases score zero for that aspect.
